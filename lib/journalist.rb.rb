...
count = 0
ary.each do |m|
  if m.length < count && m.length != 0 || count == 0
    count = m.length
  end
end
puts count
...
puts ary.sort {|m,n| m.length <=> n.length }
puts "-" * 20

#Quelle est la position dans l'array de la personne @epenser ?
puts epenser = ary.index("@epenser")
puts "-" * 20
...
ary_sort = []
ary.each do |m|
  ary_sort.push(m.length)
end
counter = ary_sort.sort.max + 1

counter.times do |i|
  if ary_sort.count(i) != 0
    if ary_sort.count(i) == 1
      puts "Il y a #{ary_sort.count(i)} handle qui fait #{i} caractères de long !"
    else
      puts "Il y a #{ary_sort.count(i)} handle(s) qui font #{i} caractères de long !"
    end
    new_hash.store(i,ary_sort.count(i))
  end
end
puts new_hash
